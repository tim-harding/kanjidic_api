 ...
 */
export type Reading = Reading_PinYin | Reading_Kunyomi | Reading_String;

export function isReading(value: unknown): value is Reading {
  return isSum(value) && isTypeFromTagged(value, CHECKERS);
}

export function isReadingStringFromTag(
  reading: Reading
): reading is Reading_String {
...
function isReadingPinYin(value: Sum): value is Reading_PinYin {
  return PinYin.check(value.content);
}
